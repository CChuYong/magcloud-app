...
class ProfileView extends BaseView<ProfileView, ProfileViewModel, ProfileViewState> {
  final User user;
  final bool isMe;
  ...
  @override
  Widget render(BuildContext context, ProfileViewModel action, ProfileViewState state) {
    return BaseSettingLayout(
        title: isMe ? message('my_profile') : message('friend_profile').format([user.name]),
        child: SizedBox(
      ...
      children: [
        ...
        Text(user.name,
            style:
            TextStyle(color: BaseColor.warmGray700, fontSize: 24.sp)),
        Text(user.nameTag,
            style:
            TextStyle(color: BaseColor.warmGray500, fontSize: 18.sp)),
        SizedBox(height: 10.sp),
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            isMe ? button(message('generic_change_profile_image'), () {}) : button(message('generic_request_friend'), (){}),
            SizedBox(width: 10.sp),
            button(message('generic_copy_tags'), () => action.copyTags(user.nameTag)),
          ],
        )
      ],
    )));
